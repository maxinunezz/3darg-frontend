...
import { Instagram, Truck, Star, Zap, Shield, ArrowRight, ChevronRight } from "lucide-react";
...
const FEATURES = [
  {
    ...
    desc: "Cada pieza fabricada capa por capa con tecnología de punta.",
  },
  {
    icon: <Star className="w-6 h-6" />,
    title: "Diseño personalizable",
    desc: "Tu logo, tus colores. Productos únicos para tu marca o entrenamiento.",
  },
  {
    icon: <Truck className="w-6 h-6" />,
    title: "Envíos a todo el país",
    desc: "Despachamos a cualquier punto de Argentina en 24-72 hs.",
  },
  {
    icon: <Shield className="w-6 h-6" />,
    title: "Garantía de calidad",
    desc: "Si no quedás conforme, lo rehacemos. Sin preguntas.",
  },
];
...
export default async function BrandPage({
  ...
  // Categories that actually have products for this brand
  const brandCategorySlugs = new Set(
    allProducts.map((p) => p.category?.slug).filter(Boolean)
  );
  const brandCategories = categories.filter((c) =>
    brandCategorySlugs.has(c.slug)
  );
  ...
  return (
    <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {featured.map((product) => (
              <Link
                ...
                className="group relative bg-card border border-border rounded-2xl overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
              >
                <div className="aspect-square bg-muted relative overflow-hidden">
                  {product.images?.[0]?.image ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-5xl text-muted-foreground/20">
                      🏋️
                    </div>
                  )}
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-300" />
                  <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="bg-white text-black text-xs font-bold px-3 py-1.5 rounded-full shadow">
                      Ver producto
                    </span>
                  </div>
                </div>
      ...
      {/* ── 4. SHOP BY CATEGORY ──────────────────────────────────────────── */}
      {hasShop && brandCategories.length > 0 && (
        <section className="bg-muted/30 py-20">
          <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              <Link
                href={`/${slug}/shop`}
                className="group relative overflow-hidden rounded-2xl bg-primary text-primary-foreground p-6 h-32 flex flex-col justify-between hover:opacity-90 transition-all"
              >
        ...
        <section className="relative overflow-hidden bg-black text-white py-24 md:py-36">
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-4 text-center md:text-left">
            <p className="text-xs font-bold uppercase tracking-widest text-primary/80 mb-4">
              Llevá tu entreno al siguiente nivel
            </p>
            <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none mb-6">
              Entrenás duro.<br />
              <span className="text-primary">Merecés lo mejor.</span>
            </h2>
            <p className="text-lg text-white/70 mb-10 max-w-xl">
              Productos diseñados por y para personas que se toman en serio su entrenamiento.
              Calidad que se nota en cada repetición.
            </p>
            <Link
              href={`/${slug}/shop`}
              className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-10 py-4 rounded-full font-bold text-lg hover:opacity-90 transition-opacity shadow-xl group"
            >
              Ver colección
      ...
      <section className="bg-muted/40 py-16">
        <div className="max-w-xl mx-auto px-4 text-center">
          ...
          <p className="text-muted-foreground mb-8 text-sm">
            Ofertas exclusivas, nuevos productos y contenido fitness antes que nadie.
          </p>
          <NewsletterForm />
